...
main:
	...
	mtc1 $t4, $f4
	cvt.s.w $f4, $f4	#convert $t4 to single values $f4, max
	
	mtc1 $t5, $f5
	cvt.s.w $f5, $f5	#convert $t5 to single values $f5
...
loop: #begin the loop
	...
	bgt $f6, $f4, max #check whether it is greater than the max value
	blt $f6, $f5, min #check if it is less than the min value
	j next
